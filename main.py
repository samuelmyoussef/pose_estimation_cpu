...
def analyse_image_endpoint():
    try:
        logger.debug('Received POST for /analyse_image')
        ...
        if image and allowed_images(image.filename):
            ...
            success, result = analyse_image(algorithm, image_path, opWrapper)
...
def analyse_video_endpoint():
    try:
        logger.debug('Received POST for /analyse_video')
        ...
        if video and allowed_videos(video.filename):
            ...
            success, result = analyse_video(algorithm, video_path, results_per_second, opWrapper)
            logger.debug('Algorithm result: {}, {}'.format(success, result))
